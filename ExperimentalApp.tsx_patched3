    ...
    const handleResize = useCallback(() => {
        ...
        if (recordingTarget) {
            ...
            if (v && v.videoWidth > 0 && v.videoHeight > 0) {
                finalW = v.videoWidth;
                finalH = v.videoHeight;
                lastVideoSizeRef.current = { w: finalW, h: finalH };
            } else if (lastVideoSizeRef.current.w > 0 && lastVideoSizeRef.current.h > 0) {
                // Fallback when metadata isn't ready yet but we already know the last clip size.
                finalW = lastVideoSizeRef.current.w;
                finalH = lastVideoSizeRef.current.h;
            } else {
                // Safe default - avoid using full window which makes 'native' look like 'fit'.
                finalW = 1280;
                finalH = 720;
            }
            ...
            <video
                ...
                onEnded={handleVideoEnded}
                 onLoadedMetadata={() => {
                     const v = videoRef.current;
                     if (v && v.videoWidth > 0 && v.videoHeight > 0) {
                         lastVideoSizeRef.current = { w: v.videoWidth, h: v.videoHeight };
                     }
                     handleResize();
                 }}
