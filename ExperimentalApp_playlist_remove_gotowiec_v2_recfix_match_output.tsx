...
const removePlaylistItem = useCallback((index: number) => {
    ...
    const startMediaRecorderRecording = async (preset: RecordingPreset, preferWebCodecs = false): Promise<boolean> => {
        ...
        const recW = preset.id === 'match_output' ? (baseCanvas?.width ?? preset.width) : preset.width;
        const recH = preset.id === 'match_output' ? (baseCanvas?.height ?? preset.height) : preset.height;
