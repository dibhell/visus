...
const Credits: React.FC = () => (
    <div className="fixed bottom-3 left-4 z-[120] text-[10px] text-slate-200 bg-black/70 border border-white/10 px-4 py-2 rounded-full backdrop-blur pointer-events-none flex items-center gap-2">
        <span className="opacity-90">Studio Popłoch (c) 2025 | Pan Grzyb | </span>
                                ...
                                >
                                    {RECORDING_VIDEO_PRESETS.map((preset) => (
                                        <option key={preset.id} value={preset.id} className="bg-slate-900 text-slate-100">
                                            {preset.label} — {(preset.width && preset.height) ? `${preset.width}x${preset.height}` : 'AUTO'} — {preset.fps} fps — {toMbps(preset.videoBitrate)} Mb/s / {toKbps(preset.audioBitrate)} kbps
                                        </option>
                                    ))}
...
const removePlaylistItem = useCallback((index: number) => {
    ...
    const startMediaRecorderRecording = async (preset: RecordingPreset, preferWebCodecs = false): Promise<boolean> => {
        ...
        const useSource = preset.id === 'match_source' && sourceW > 0 && sourceH > 0;
        const useOutput = preset.id === 'match_output' || preset.width === 0 || preset.height === 0;
        const recW = useSource ? sourceW : (useOutput ? (baseCanvas?.width ?? 1920) : preset.width);
        const recH = useSource ? sourceH : (useOutput ? (baseCanvas?.height ?? 1080) : preset.height);
        const { canvas: recCanvas, ctx } = ensureRecordingCanvas(recW, recH);
        if (!ctx) {
            alert('Recording canvas unavailable.');
            return false;
        }
        ctx.drawImage(canvasRef.current, 0, 0, recW, recH);
        ...
        console.info('[VISUS] record start', {
            ...
            videoSource: {
                ...
                intrinsic: { w: videoRef.current?.videoWidth || 0, h: videoRef.current?.videoHeight || 0 }
            },
            debugNoAudio
        });
