...
const resizeMeasuredRef = useRef<ResizeState | null>(null);
...
const measureResizeOnly = useCallback((): ResizeState | null => {
    ...
    // READ ONLY: panel width (layout read)
    const panelWidth = (panelVisible && uiPanelRef.current)
        ? uiPanelRef.current.getBoundingClientRect().width
        : 0;
    ...
    // Determine target output resolution (READ ONLY)
    let finalW = 1080;
    let finalH = 1920;

    if (isRecording) {
        finalW = recordResolution.width;
        finalH = recordResolution.height;
    } else if (aspectRatio === '16:9') {
        finalW = 1920; finalH = 1080;
    } else if (aspectRatio === '9:16') {
        finalW = 1080; finalH = 1920;
    } else if (aspectRatio === '4:3') {
        finalW = 1440; finalH = 1080;
    } else if (aspectRatio === '3:4') {
        finalW = 1080; finalH = 1440;
...
const applyResizeOnly = useCallback((s: ResizeState) => {
    // WRITE ONLY: DOM styles + renderer/worker resize
    const canvas = canvasRef.current;
    if (!canvas) return;

    setIsMobile(s.isMobileNow);
    ...
    if (useWorkerRenderRef.current && workerRef.current) {
        workerRef.current.postMessage({ type: 'resize', width: s.renderW, height: s.renderH });
    } else {
        rendererRef.current?.resize(s.renderW, s.renderH);
    }
}, [setIsMobile]);

const requestResizeApply = useCallback(() => {
...
const requestResizeMeasure = useCallback(() => {
    if (resizeMeasurePendingRef.current) return;
    resizeMeasurePendingRef.current = true;

    // NOTE: measure runs in microtask, apply runs in rAF (separate phase)
    queueMicrotask(() => {
        resizeMeasurePendingRef.current = false;
        const s = measureResizeOnly();
        if (!s) return;
        resizeMeasuredRef.current = s;
        requestResizeApply();
    });
}, [measureResizeOnly, requestResizeApply]);

const handleResize = useCallback(() => {
    requestResizeMeasure();
}, [requestResizeMeasure]);
    ...
    useEffect(() => {
        ...
        const loop = (t: number) => {
            ...
            } else if (canUseWebGL && renderModeRef.current === 'webgl-fastgl' && videoRef.current && (rendererRef.current?.isReady?.() === true)) {
