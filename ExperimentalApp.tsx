...
const InitOverlay: React.FC<{ onInit: () => void }> = ({ onInit }) => {
    return (
        <div
            style={{
                position: 'fixed',
        ...
        >
            <div style={{ maxWidth: 520, width: '100%', textAlign: 'center' }}>
                <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 10 }}>VISUS</div>
                <div style={{ opacity: 0.85, marginBottom: 18, lineHeight: 1.4 }}>
                    Kliknij INITIALIZE, żeby uruchomić audio i silnik renderowania (wymaga gestu użytkownika).
                </div>
                <button
                    type="button"
                    onClick={onInit}
                ...
                <div style={{ marginTop: 10, opacity: 0.6, fontSize: 12 }}>
                    Jeśli przeglądarka blokuje dźwięk, to ten przycisk to „odblokowanie”.
                </div>
            </div>
        </div>
    );
...
const ExperimentalApp: React.FC<ExperimentalProps> = (props) => {
    ...
    return (
        <VisusErrorBoundary>
            {!bootRequested ? (
                <InitOverlay onInit={() => setBootRequested(true)} />
            ) : (
                <ExperimentalAppFull {...props} bootRequested={bootRequested} />
            )}
        </VisusErrorBoundary>
    );
};
