...
const ExperimentalAppFull: React.FC<ExperimentalProps> = ({ onExit, bootRequested = false }) => {
    ...
    const resizeMeasuredRef = useRef<null | {
        isMobile: boolean;
        displayW: number;
        displayH: number;
        left: number;
    ...
    const resizeMeasurePendingRef = useRef(false);
    const resizeApplyPendingRef = useRef(false);
    ...
    const addCleanup = useCallback((fn: () => void) => {
        ...
        const wrapped = () => {
            if (!active) return;
            active = false;
            const idx = cleanupFnsRef.current.indexOf(wrapped);
            if (idx >= 0) cleanupFnsRef.current.splice(idx, 1);
            try {
                fn();
            } catch (err) {
                console.warn('[VISUS] cleanup failed', err);
            }
        };
        cleanupFnsRef.current.push(wrapped);
        return wrapped;
    }, []);

    const runAllCleanup = useCallback(() => {
        const fns = cleanupFnsRef.current.splice(0);
        ...
        return () => {
            ...
            if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
                try {
                    ...
                    mediaRecorderRef.current.onerror = null as any;
            ...
            try { stopRecordingCopyLoop(); } catch {}
            try { stopLoop(); } catch {}
            try {
    ...
    }, [bootRequested, debugFlagSet.noAudio, debugFlagSet.noGL, debugFlagSet.noWorker, runAllCleanup]);
    ...
    const measureResizeOnly = useCallback((): ResizeState | null => {
        ...
        const left = isMobileNow
            ? (wWindow - displayW) / 2
            : panelWidth + sideGap + (availableW - displayW) / 2;
        const top = topOffset + (availableH - displayH) / 2;
    ...
    }, [aspectRatio, isRecording, panelVisible, recordResolution.height, recordResolution.width]);

    const applyResizeOnly = useCallback((s: ResizeState) => {
        const canvas = canvasRef.current;
        if (!canvas) return;
        setIsMobile(s.isMobile);
        canvas.style.width = `${s.displayW}px`;
        canvas.style.height = `${s.displayH}px`;
        canvas.style.left = `${s.left}px`;
        canvas.style.top = `${s.top}px`;
        applyRendererResize(s.renderW, s.renderH);
    }, [applyRendererResize]);

    const requestApply = useCallback(() => {
        if (resizeApplyPendingRef.current) return;
        resizeApplyPendingRef.current = true;
        requestAnimationFrame(() => {
            resizeApplyPendingRef.current = false;
            if (disposedRef.current) return;
            const s = resizeMeasuredRef.current;
            if (!s) return;
            applyResizeOnly(s);
        });
    }, [applyResizeOnly]);
    ...
    const requestMeasure = useCallback(() => {
        if (resizeMeasurePendingRef.current) return;
        resizeMeasurePendingRef.current = true;
        const schedule = typeof queueMicrotask === 'function'
            ? queueMicrotask
            : (cb: () => void) => Promise.resolve().then(cb);
        schedule(() => {
            resizeMeasurePendingRef.current = false;
            if (disposedRef.current) return;
            const measured = measureResizeOnly();
            if (!measured) return;
            resizeMeasuredRef.current = measured;
            requestApply();
        });
    }, [measureResizeOnly, requestApply]);
    ...
    }, [lockResolution, quality, requestMeasure]);
    ...
    const loopModeRef = useRef<'raf' | 'rvfc'>('raf');
    ...
    const stopLoop = useCallback(() => {
        ...
        if (videoEl && videoFrameRequestRef.current !== null && typeof videoEl.cancelVideoFrameCallback === 'function') {
            try { videoEl.cancelVideoFrameCallback(videoFrameRequestRef.current); } catch {}
        }
        videoFrameRequestRef.current = null;
        lastFrameRef.current = 0;
    }, []);

    const startLoop = useCallback((mode: 'raf' | 'rvfc') => {
        if (debugNoLoop) return;
        if (loopRunningRef.current && loopModeRef.current === mode) return;

        stopLoop();
        loopModeRef.current = mode;
        loopRunningRef.current = true;
        console.info('[VISUS] loop start', { mode });

        const scheduleNext = () => {
            if (!loopRunningRef.current) return;
            const videoEl: any = videoRef.current;
            if (mode === 'rvfc' && videoEl && typeof videoEl.requestVideoFrameCallback === 'function') {
        ...
        scheduleNext();
    }, [debugNoLoop, requestMeasure, stopLoop]);
    ...
    useEffect(() => {
        ...
        const want: 'raf' | 'rvfc' = useVideoFrameCb ? 'rvfc' : 'raf';
        startLoop(want);
        return () => stopLoop();
    }, [debugNoLoop, startLoop, stopLoop, useVideoFrameCb]);
