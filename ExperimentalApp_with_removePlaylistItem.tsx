...
const removePlaylistItem = useCallback((index: number) => {
    // Minimal, safe remove: keep indices consistent and avoid requiring full app reset.
    setPlaylist(prev => {
        ...
        // Revoke blob URL if it's not referenced anymore (and not currently used by <video> src).
        try {
            if (removed?.url?.startsWith('blob:')) {
                const stillUsed = next.some(p => p.url === removed.url);
                const videoSrc = videoRef.current?.src ?? '';
                if (!stillUsed && videoSrc !== removed.url) {
        ...
        if (cur == null) {
            ...
            // removed currently playing: pick item that slid into this index, else previous, else none
            if (next.length === 0) newCur = null;
            else if (index < next.length) newCur = index;
            else newCur = next.length - 1;
        } else if (cur > index) {
            // removed item before current -> shift left
            newCur = Math.max(0, cur - 1);
        }

        // Apply new current/next + handle playback changes outside of the setPlaylist reducer tick
        queueMicrotask(() => {
            setPlaylistCurrentIndex(newCur);

            if (newCur == null) {
                setPlaylistNextIndex(null);
                // stop video if it was playing from playlist
                try {
                    if (videoRef.current) {
                        videoRef.current.pause();
                        videoRef.current.srcObject = null;
            ...
            const nextIdx = pickNextPlaylistIndex(
                next,
                playlistModeRef.current,
                playlistAvoidRepeatRef.current,
                newCur
            );
            setPlaylistNextIndex(nextIdx);

            // If we removed the currently playing item, load the new current.
            if (cur === index) {
                setTimeout(() => {
                    loadPlaylistClip(newCur!);
                }, 0);
            }
        });
