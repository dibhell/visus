    ...
    const removePlaylistItem = useCallback(
        (index: number) => {
            setPlaylist((prev) => {
                ...
                // Revoke removed blob URL to avoid leaks.
                if (removed?.url && removed.url.startsWith('blob:')) {
                    try { URL.revokeObjectURL(removed.url); } catch {}
                }

                // Adjust indices (current/next) so UI + logic stay consistent.
                const prevCurrent = playlistCurrentIndexRef.current;
                const prevNext = playlistNextIndexRef.current;

                let nextCurrent: number | null = prevCurrent;
                let nextNext: number | null = prevNext;

                const removedWasCurrent = prevCurrent === index;

                if (nextCurrent != null) {
                    if (index < nextCurrent) nextCurrent = nextCurrent - 1;
                    else if (index === nextCurrent) nextCurrent = null;
                }

                if (nextNext != null) {
                    if (index < nextNext) nextNext = nextNext - 1;
                    else if (index === nextNext) nextNext = null;
                }

                setPlaylistCurrentIndex(nextCurrent);
                setPlaylistNextIndex(nextNext);

                // If we removed the currently playing item, load a sensible replacement.
                if (removedWasCurrent && nextList.length > 0) {
                    const replacementIndex = Math.min(index, nextList.length - 1);
                    setTimeout(() => loadPlaylistClip(replacementIndex), 0);
                }
                        ...
                        {playlist.length > 0 && (
                            <div className="mt-2 max-h-24 overflow-y-auto custom-scrollbar text-[9px] bg-black/20 rounded-xl border border-white/5">
                                {playlist.map((item, idx) => (
                                    <div
                                        key={item.id}
                                        className={`w-full flex items-center justify-between px-3 py-1.5 border-b border-white/5 last:border-b-0 ${
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            className="ml-2 w-6 h-6 rounded flex items-center justify-center text-slate-400 hover:text-red-300 hover:bg-white/5"
